...
d30 = read.csv('outputdata/datachi_5Variables_APRESENTACAO.csv')
d30[1] =  NULL

names(d30)[3] = 'value'
names(d30)[2] = 'individual'
names(d30)[1] = 'Característica'
...
data$Característica = fct_rev(data$Característica)

# Set a number of 'empty bar' to add at the end of each Característica
empty_bar <- 0
to_add <- data.frame( matrix(NA, empty_bar*nlevels(data$Característica), ncol(data)) )
colnames(to_add) <- colnames(data)
to_add$Característica <- rep(levels(data$Característica), each=empty_bar)
data <- rbind(data, to_add)
data <- data %>% arrange(Característica)
...
# Make the plot
p <- ggplot(data, aes(x=as.factor(id), y=value, fill=Característica)) +       # Note that id is a factor. If x is numeric, there is some space between the first bar
