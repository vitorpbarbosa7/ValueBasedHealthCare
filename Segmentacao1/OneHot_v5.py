...
data = pd.read_csv('indata/data.csv')
...
#Sexo
sexo = pd.DataFrame(pd.get_dummies(data3['Sexo'])).astype('Int64')
sexo.columns = ['Masc','Fem']
sexo = dumNaN(sexo.copy())

#Estado Civil
civil = pd.DataFrame(pd.get_dummies(data3['Estado_Civil'])).astype('Int64')
civil.columns = ['Casado_Amasiado','Solteiro','Separado_Divorciado','4','5','6']
civil = dumNaN(civil.copy())

#Sintoma livrar (#CUIDADO PORQUE ESSE ESTAVA SEM O 4!!!)
sintomalivrar = pd.DataFrame(pd.get_dummies(data3['Qual_sintoma'])).astype('Int64')
sintomalivrar.columns = ['Livrar_costas','Livrar_perna','Livrar_perda_sens']
...
data7.to_csv('indata/data7toR.csv')
