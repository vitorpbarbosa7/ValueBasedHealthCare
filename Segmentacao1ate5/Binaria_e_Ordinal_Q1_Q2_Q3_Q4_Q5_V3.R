...
Bin_Gut_BPI_RMDQ = read.csv('indata/Bin_Got_BPI_RMDQ.csv')

#Ordinais até RMDQ
Ord_Gut_BPI_Oswestry_RMDQ = read.csv('indata/Ordinal_Q1_Q2_Q3_Q4.csv', sep = ',')
Ord_Gut_BPI_Oswestry_RMDQ[c(1:2)] = NULL

#Carregar as novas ordinárias de A SUA SAÚDE E BEM-ESTAR
library(foreign)
data_SPSS = read.spss('indata/Banco.sav', to.data.frame = T)

data_SF = data_SPSS[c(77:88)]

#Inverter a ordem dos níveis dos fatores de 9 até 12:
for (i in 9:12){
  
  data_SF[[i]] = fct_rev(data_SF[[i]])
  
}
#Transformar em numérico (Como LabelEncoder)
for (i in 1:length(data_SF)){
  
  data_SF[[i]] = as.numeric(data_SF[[i]])
}

#Salvando todos ordinais até o momento:
Ord_Gut_BPI_Oswestry_RMDQ_SF = data.frame(cbind(Ord_Gut_BPI_Oswestry_RMDQ,data_SF))
